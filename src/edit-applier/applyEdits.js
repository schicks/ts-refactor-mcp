...
async function validateEdits(edits) {
    for (const fileEdit of edits) {
        ...
        try {
            ...
            throw new Error(`Cannot read file ${fileEdit.filePath}: ${error instanceof Error ? error.message : String(error)}`);
        }
        // Validate edit positions
        for (const edit of fileEdit.textEdits) {
            if (edit.start.line < 1 || edit.start.offset < 1) {
                throw new Error(`Invalid edit position in ${fileEdit.filePath}: line ${edit.start.line}, offset ${edit.start.offset}`);
            }
            if (edit.end.line < edit.start.line) {
                throw new Error(`Invalid edit range in ${fileEdit.filePath}: end line ${edit.end.line} before start line ${edit.start.line}`);
            }
            if (edit.end.line === edit.start.line &&
                edit.end.offset < edit.start.offset) {
                throw new Error(`Invalid edit range in ${fileEdit.filePath}: end offset ${edit.end.offset} before start offset ${edit.start.offset}`);
            }
        }
    }
}
 ...
 */
export async function applyEditsAndMoveFile(edits, oldPath, newPath) {
